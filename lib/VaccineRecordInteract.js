...
const CONTRACT_ADDRESS = "0xAcaACa347Be13241104B4ff65bA74aE7C1e0C0cC";
...
export const getProfileFromChain = async (provider, userAddress) => {
    try {
      const contract = await getContractInstance(provider); // Add await here
      const documentHash = await contract.getPatientVaccineProfile(userAddress);
